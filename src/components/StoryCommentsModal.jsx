...
export default function StoryCommentsModal({ visible, onClose, storyId, navigation }) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const data = await getComments(storyId, {}, 'stories')
  ...
  const handlePost = async () => {
    ...
    if (activeReply) {
      ...
      try {
        const addedReply = await addReply(storyId, activeReply.commentId, newCommentData, 'stories')
        setRepliesData(prev => ({
          ...prev,
      ...
      try {
        const addedComment = await addComment(storyId, newCommentData, 'stories')
  ...
  const handleLike = async (commentId, isLiked) => {
    ...
    try {
      await toggleLikeComment(storyId, commentId, currentUid, 'stories')
    } catch (error) {
  ...
  const loadReplies = async (commentId) => {
    ...
    try {
      const fetchedReplies = await getReplies(storyId, commentId, 'stories')
    ...
    const handleProfilePress = () => {
      if (navigation) {
        ...
        if (item.userId === currentUid) {
          ...
          setTimeout(() => navigation.navigate('ProfileScreen', { userId: item.userId }), 250)
        }
      }
    }
